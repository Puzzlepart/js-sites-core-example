...
module Pzl.Workspace {        
   ...
   function GetWebProperties() {
    ...
    function GetWelcomePageProperties(startsWith? : string, useDisplayNames? : boolean, excludeProps = []) {
        var def = jQuery.Deferred();
        var clientContext = SP.ClientContext.get_current();
        var list = clientContext.get_web().get_lists().getByTitle("Site Pages");
        var fields : SP.FieldCollection = null;
            ...
            () => {
                ...
                if(startsWith) {
                    ...
                    Object.keys(allProperties).forEach(key => {
                        if(key.substring(0,startsWith.length) != startsWith || Array.contains(excludeProps, key)) return;
    ...
    function GetWebPartElement(wpTitle : string) {
        ...
        ExecuteOrDelayUntilScriptLoaded(() => {
            GetWelcomePageProperties("Port", true).then((properties) => {
                ...
                for(var key in properties) { 
                    if(key == "Document Space Active") continue;
                    else if(!properties[key]) continue;
                    else if(properties[key].Label) metadata[key] = properties[key].Label;
                    else if(properties[key].get_label) metadata[key] = properties[key].get_label();
                    else metadata[key] = properties[key];
                }
    ...
    export function UpdateWebPropertyBag(key, value) {
        var def = jQuery.Deferred();
